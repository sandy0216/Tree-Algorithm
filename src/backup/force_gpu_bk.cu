...
__device__ void force_gpu(GNODE *root,double *x,double *y,double *mass,double *fx,double *fy,double *V,int *region_index,int *thread_load,unsigned int *regnum)
{
...
__device__ void force_share(double *x,double *y,double *mass,double *fx,double *fy,double *V,int *thread_load,unsigned int *regnum)
{
	int nx = blockDim.x*gridDim.x;
	int ix = blockDim.x*blockIdx.x+threadIdx.x;
	int iy = blockDim.y*blockIdx.y+threadIdx.y;
	...
	for( int i=st_reg;i<thread_load[thread_id];i++ ){
		...
		for( int j=0;j<par_n;j++ ){
			...
			loc_fy = 100;
			loc_v = 0;
			node_share_force_gpu(0,x[par_id],y[par_id],mass[par_id],&loc_fx,&loc_fy,&loc_v);
			fx[par_id] = loc_fx;
			fy[par_id] = loc_fy;
			V[par_id] = loc_v;
		}
...
__device__ void node_share_force_gpu(int cache_id,double x,double y,double mass,double *fx,double *fy,double *v)
{
	...
	n = cache[cache_id].num;
	if( n!=0 ){
	r = sqrt((cmx-x)*(cmx-x)+(cmy-y)*(cmy-y));
